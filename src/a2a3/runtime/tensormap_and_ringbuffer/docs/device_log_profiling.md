...
|-------|-------------|-------------|
| **complete** | Polls handshake on each managed core; when a core completes, calls `on_task_complete(task_id)` which traverses fanout list (notify consumers) and fanin list (release producers) in a single pass | `fanout`: edges/max_degree/avg for consumer notification; `fanin`: edges/max_degree/avg for producer release |
| **scan** | Updates the perf profiling header with latest scheduler state | — |
| **dispatch** | Batch collects ready tasks from unified ready queue, dispatches to idle cores matching `core_type`, builds the dispatch payload, and writes the task to the core's handshake register | `pop`: `hit` = successful pops (task dispatched), `miss` = empty queue pops, `hit_rate` = hit/(hit+miss) |
